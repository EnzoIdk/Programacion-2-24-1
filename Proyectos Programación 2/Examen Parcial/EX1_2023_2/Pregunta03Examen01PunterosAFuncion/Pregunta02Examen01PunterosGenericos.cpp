...
void pruebaDeCargaDeProductos(void *prod){
    ...
    for(int i=0;productos[i];i++){
        ...
        imprimirDatosProductos(arch,aux[0],i+1);
    }
}

void imprimirDatosProductos(ofstream &arch,void *dat,int num){
    void **datos=(void**)dat;
    char *codigo=(char*)datos[0],*desc=(char*)datos[1];
    double *precio=(double*)datos[2];
    int *stock=(int*)datos[3];
    arch<<"Producto "<<num<<':'<<endl<<codigo<<setw(3)<<' '<<left<<setw(60)<<
            desc<<right<<endl<<"Precio: "<<setw(10)<<*precio<<setw(7)<<' '<<
            "Stock: "<<setw(3)<<*stock<<endl;
}
