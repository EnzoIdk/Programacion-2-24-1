...
bool Estantes::operator+=(class Libro &lib){
    ...
    lib.SetColocado(pos != NO_ESPACIO);
    AnadirLibro(lib);
    return pos != NO_ESPACIO;
}
int Estantes::HayEspacio(class Libro &lib){
    int i,num;
    for(i = 0;i < anchura;i++)
        if(espacios[i].GetContenido() == ' ')num++;
    if(lib.GetAncho() <= num)return (anchura - num)-1;
    return NO_ESPACIO;
}
...
ofstream & operator<<(ofstream &arch, const class Estantes &Est){
    ...
    arch << "Codigo Estante: " << codigo << setw(20) << "Cantidad de Libros: "
