...
void Alumno::ObtenerDatos(){
    char codCurso[8],codCursoAux[8];
    for(int i=0;i<numcursos;i++){
        int num=0;
        lnotas[i].GetCodcurso(codCurso);
        for(int j=0;j<numcursos;j++){
            lnotas[j].GetCodcurso(codCursoAux);
            if(strcmp(codCurso,codCursoAux)==0) num++; 
        }
        if(lnotas[i].GetNota()>10) numaprobados++;
        if(num==1) numprimera++;
        else if(num==2) numsegunda++;
        else if(num==3) numtercera++;
    }
    numsegunda/=2;
    numtercera/=3;
}